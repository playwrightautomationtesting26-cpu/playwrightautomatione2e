...
test.describe(
  'Make Appointment Functionality',
  {
    annotation: {
      type: 'Story',
      description: 'JIRA-1234: Test the make appointment functionality',
    },
  },
  () => {
    test.beforeEach(
      ...
      async ({ page }, testInfo) => {
        ...
        await expect(
          page.locator("h2:has-text('Make Appointment')"),
        ).toBeVisible()

        /**
         * Add custom screenshot at test step level
         * @TODO: add this as a utility function
         **/
        await pwHelper.takeFullPageScreenshot(page, 'login-successful')
        // let fullPageScreenshot = await page.screenshot({ fullPage: true })
        // await testInfo.attach('login-successful', {
    ...
    test(
      'Should book an appointment successfully',
      {
        annotation: {
          type: 'Bug',
          description: 'Defect: 124 - Does not work in firefox ',
        },
      ...
      async ({ page, browserName }) => {
        ...
        test.skip(
          browserName === 'firefox',
          'open Bug id: 124 in JIRA for more details',
        )
        await page
          .getByLabel('Facility')
